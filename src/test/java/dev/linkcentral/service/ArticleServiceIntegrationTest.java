...
public class ArticleServiceIntegrationTest {
    ...
    void find_article_by_id() {
        ...
        // when
        ArticleRequestDTO foundArticle = articleService.findById(article.getId(), member); // 저장한 Article의 ID를 사용하여 조회
    ...
    void toggle_like_when_like_exists() {
        ...
        // when
        articleService.toggleLike(article.getId(), member);
    ...
    void get_article_likes_count() {
        ...
        // when
        int likesCount = articleService.getLikesCount(article.getId());
